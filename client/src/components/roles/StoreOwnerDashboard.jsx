...
const StoreOwnerDashboard = () => {
    ...
    if (!dashboardData) return <p className="text-center text-gray-600">No store data available.</p>;

    return (
        <div className="bg-white p-8 rounded-lg shadow-md">
            <h2 className="text-2xl font-bold text-gray-800 mb-2">My Store: {dashboardData.storeName}</h2>
            <div className="flex items-center text-xl mb-6">
                ...
                <div className="flex items-center bg-yellow-100 text-yellow-800 px-3 py-1 rounded-full">
                    ...
                    <span className="font-bold">{dashboardData.averageRating}</span>
                </div>
            </div>
            ...
            <div className="overflow-x-auto">
                <table className="min-w-full bg-white">
                    ...
                    <tbody className="divide-y divide-gray-200">
                        {dashboardData.ratings.map(({ _id, user, rating }) => (
                            <tr key={_id}>
            ...
            {dashboardData.ratings.length === 0 && <p className="text-center mt-4 text-gray-500">No ratings submitted yet.</p>}
        </div>
    );
};
